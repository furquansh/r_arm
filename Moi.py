...
def get_cylinder_inertia_matrix(mass, radius, height):
    """Given mass, radius, and height of a cylinder, return inertia matrix.
    :return: ixx, ixy, ixz, ixy, iyy, iyz, ixz, iyz, izz
    From https://www.wolframalpha.com/input/?i=inertia+matrix+cylinder&rawformassumption=%7B%22C%22,+%22cylinder%22%7D+-%3E+%7B%22Solid%22%7D
     """
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        ...
        print("Cylinder inertia of mass 1.0, radius 1.0, and height 1.0:")
        print(get_cylinder_inertia_matrix(1.0, 1.0, 1.0))
    elif len(sys.argv) == 3:
        print(get_sphere_inertia_matrix(float(sys.argv[1]), float(sys.argv[2])))
    elif len(sys.argv) == 4:
        print(get_cylinder_inertia_matrix(float(sys.argv[1]), float(sys.argv[2]), float(sys.argv[3])))
    elif len(sys.argv) == 5:
        print("Cube inertia matrix:")
        print(get_cube_inertia_matrix(float(sys.argv[1]), float(sys.argv[2]), float(sys.argv[3]), float(sys.argv[4])))
    else:
